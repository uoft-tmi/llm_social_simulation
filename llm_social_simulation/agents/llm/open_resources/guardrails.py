...
from .constraints import constraints_from_observation
...
class GuardrailsPolicy(OpenResourcesPolicy):
    ...
    def decide(self, obs: OpenResourcesObservation) -> OpenResourcesAction:
        ...
        constraints = constraints_from_observation(
            obs=obs,
            max_harvest_per_step=float(self.max_harvest_per_step),
            sustainable_frac=float(self.sustainable_frac),
            contrib_max_frac=float(self.contrib_max_frac),
        )
        sustainable_cap = float(constraints.sustainable_harvest_cap)
        ...
        max_contribute = float(constraints.max_contribute)
